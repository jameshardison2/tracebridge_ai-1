...
const generateDemoBoilerplate = (docType: any) => {
    let content = `=================================================================\n`;
    content += `DOCUMENT: ${docType.name.replace(/_/g, ' ')}\n`;
    content += `DEVICE: GastroCam Swallowable Endoscope\n`;
    content += `PRODUCT CODE: NEK\n`;
    content += `CLASS: Class II\n`;
    content += `VERSION: 1.0\n`;
    content += `DATE: ${new Date().toISOString().split('T')[0]}\n`;
    content += `=================================================================\n\n`;

    content += `1.0 INTRODUCTION\n`;
    content += `This document serves as the formal ${docType.name.replace(/_/g, ' ')} for the GastroCam medical device. \n\n`;
    content += `3.0 DETAILED SPECIFICATIONS\n`;

    for (let i = 1; i <= 50; i++) {
        const id = `${docType.prefix}-${i.toString().padStart(4, '0')}`;
        
        // --- THE INVESTOR DEMO "HOOK" (INTENTIONAL FAILURES) ---
        if (docType.prefix === "SBOM" && i === 5) {
            content += `[${id}] Cybersecurity Bill of Materials: Due to the rapid prototyping schedule, a formal Software Bill of Materials (SBOM) was not compiled. The engineering team assumes the open-source React and Express libraries are inherently secure. CVE vulnerability mapping will be deferred until post-market phase.\n`;
        } else if (docType.prefix === "RMR" && i === 5) {
            content += `[${id}] Risk Evaluation Method: The risk management process relies on informal team consensus during weekly standups. No formal mathematical Probability x Severity (PxS) matrix was utilized, as the device is deemed relatively safe by the lead developer.\n`;
        } 
        
        // --- THE GOLDEN EVIDENCE (PERFECT COMPLIANCE) ---
        else if (docType.prefix === "SVR" && i === 10) {
        ...
        } else if (docType.prefix === "SRS" && i === 10) {
             content += `[${id}] Physical Segregation: The Software as a Medical Device (SaMD) logic is physically segregated from the commercial operating system layer using a hypervisor and dedicated RTOS partition, ensuring consumer apps cannot interfere with medical functions.\n`;
        } else {
...
async function main() {
    const baseDir = path.join(__dirname, '..', 'demo_data', 'investor_demo', 'GastroCam_Swallowable_Endoscope');
    
    if (!fs.existsSync(baseDir)) {
        fs.mkdirSync(baseDir, { recursive: true });
    }

    console.log(`Starting generation of Investor Demo Payload...`);
    let count = 0;

    for (const doc of documentTypes) {
        const fileName = `GastroCam_Swallowable_Endoscope_${doc.name}_v1.0.txt`;
        const filePath = path.join(baseDir, fileName);
        
        const content = generateDemoBoilerplate(doc);
        fs.writeFileSync(filePath, content);
        count++;
    }

    console.log(`\n✅ Success! Generated ${count} documents for the Ultimate Investor Demo.`);
